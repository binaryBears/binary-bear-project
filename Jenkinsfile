...
pipeline {
    ...
    environment {
        ENV_TYPE = "production"
        PORT = 4141
        NAMESPACE = "01bearscommunity-ru"
        REGISTRY_HOSTNAME = "just2beeandrew"
        REGISTRY = "registry.hub.docker.com"
        PROJECT = "binary-bears-frontend"
        DEPLOYMENT_NAME = "binary-bears-frontend-deployment"
        ...
        stage('Push docker image') {
             steps {
                 echo "Push image started..."
                     script {
                          docker.withRegistry("https://${env.REGISTRY}", '01bearscommunity-ru') {
